...
const Header = () => {
  ...
  return (
    <header
      className={`transition-ease fixed top-0 left-0 z-99999 w-full py-10 ${"bg-white py-8! shadow-sm dark:bg-black"}`}
    >
      <div className="max-w-c-1390 relative mx-auto items-center justify-between px-4 md:px-8 xl:flex 2xl:px-0">
        <div className="flex w-full items-center justify-between xl:w-1/4">
          ...
          <button
            ...
            className={`mr-[30px] block scale-[1.7] bg-white p-[4px] xl:hidden dark:bg-[#202225]`}
        ...
        {/* Nav Menu Start   */}
        <div
          className={`invisible h-0 w-full items-center justify-between overflow-hidden xl:visible xl:flex xl:h-auto xl:w-full ${
            navigationOpen &&
            ...
            <ul className="flex flex-col gap-5 xl:flex-row xl:items-center xl:gap-10">
              {menuData.map((menuItem, key) => (
                <li key={key} className={menuItem.submenu && "group relative"}>
                  <Link
                    href={`${menuItem.path}`}
                    className={`hover:text-[#006bff] dark:hover:text-[#ffffff] ${
                      pathUrl === menuItem.path
                        ? "text-[#006bff] dark:text-[#ffffff]"
                        : "text-grey"
                    }`}
                  >
                    {menuItem.title}
